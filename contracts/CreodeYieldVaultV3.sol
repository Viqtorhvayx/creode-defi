 ...
 */
contract CreodeYieldVaultV3 is AccessControl, ReentrancyGuard, Pausable {
    ...
    event Compounded(address indexed user, uint256 indexed id, uint256 addA, uint256 addB);
    ...
    /**
     * @notice Fold a position's accrued yield back into its principal (real
     *         auto-compounding). ERC20 yield is pulled from the treasury into
     *         the vault's custody; HBAR yield is already held in the reserve.
     *         Callable by anyone (the owner or a keeper) for a given user.
     */
    function compound(address user, uint256 id) public nonReentrant {
        Strategy memory s = strategies[id];
        require(s.exists, "unknown");
        Position storage pos = _positions[user][id];
        _accrue(pos, s.apyBps);

        uint256 addA = pos.accruedA;
        uint256 addB = pos.accruedB;
        if (addA == 0 && addB == 0) return;

        pos.accruedA = 0;
        pos.accruedB = 0;

        // Bring ERC20 yield into custody so the larger principal is backed.
        if (s.tokenA != HBAR && addA > 0) IERC20(s.tokenA).safeTransferFrom(treasury, address(this), addA);
        if (s.tokenB != s.tokenA && s.tokenB != HBAR && addB > 0) IERC20(s.tokenB).safeTransferFrom(treasury, address(this), addB);

        pos.amtA += addA;
        if (s.tokenB != s.tokenA) pos.amtB += addB;

        emit Compounded(user, id, addA, addB);
    }

    /// @notice Compound the caller's own position.
    function compoundMine(uint256 id) external {
        compound(msg.sender, id);
    }
