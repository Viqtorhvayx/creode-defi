...
// the handful of tokens whose Lazer feed actually supports the "real_time"
// channel (confirmed live against https://pyth.dourolabs.app/v1/symbols
// while building this): BTC, ETH, SOL, HYPE, XRP, BNB, DOGE, PEPE. Every
// other vault-watch token is capped at Lazer's fixed_rate@200ms (or 50ms for
...
//
// If PYTH_LAZER_API_KEY isn't set, or the free tier doesn't actually permit
// the real_time channel, this connects, logs why, and does nothing further
// — server.js's Binance path is completely unaffected either way.
...
const idToSym = Object.fromEntries(
  Object.entries(REAL_TIME_FEEDS).map(([sym, f]) => [f.id, sym])
);
...
function connectPythLazer(broadcast) {
  ...
  ws.on('open', () => {
    console.log('[pyth-lazer] connected, subscribing to', Object.keys(REAL_TIME_FEEDS).join(', '));
    ws.send(JSON.stringify({
      type: 'subscribe',
      subscriptionId: 1,
      priceFeedIds: Object.values(REAL_TIME_FEEDS).map((f) => f.id),
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (msg.type === 'subscriptionError' || msg.type === 'error') {
        // Expected outcome if the free tier doesn't permit real_time — logged
      ...
      for (const feed of msg.parsed.priceFeeds || []) {
        ...
        const meta = sym && REAL_TIME_FEEDS[sym];
