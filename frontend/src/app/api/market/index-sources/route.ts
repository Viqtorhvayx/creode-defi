 ...
 *
 * Reads the CEX spot books that perp DEX oracles are computed FROM, and returns
 * all of them in one response so the client can apply a venue's own published
 * formula itself.
 *
 * Why this exists: a perp DEX's oracle price is not an independent observation.
 * It is a median across public CEX books, recomputed and republished on a fixed
 * drumbeat — every ~505ms on Gains, every 3s on Hyperliquid. Between publishes
 * the number is frozen while the inputs keep moving. Reading the same inputs
 * continuously and applying the same function gets to their next value before
 * they print it. The lead comes from their publish interval, not from beating
 * any exchange, and it is therefore bounded by that interval.
 *
 * Server-side because none of these send CORS headers for browser calls.
 * Sources that fail or are blocked from the deploy region are reported in
 * `missing` and dropped from the median, which is what the venues themselves do
 * with a dead feed.
 *
 * runtime='edge' matches cex-fallback — measured there at ~165-170ms against
 * ~400-500ms for the Node serverless path, and this route sits on the same
 ...
 * sat $72.21 BELOW Binance's USDT book on a $76.6k price — 9.4bp — and
 * USDT/USD at 0.999055 accounts for $72.4 of it. Reporting a USDT median as a
 * USD price would be wrong by more than nine times Gains' own quoted spread.
 * Coinbase first, Kraken as the backup; both quote the pair directly. */
async function usdtUsd(): Promise<number | null> {
  try {
    const r = await fetch('https://api.exchange.coinbase.com/products/USDT-USD/ticker', {
      cache: 'no-store',
      headers: { 'User-Agent': 'creode' },
    });
    if (r.ok) {
      const d = await r.json();
      const m = mid(d?.bid, d?.ask);
      if (m && m > 0.9 && m < 1.1) return m;
    }
  } catch { /* fall through to Kraken */ }
  try {
    const r = await fetch('https://api.kraken.com/0/public/Ticker?pair=USDTZUSD', { cache: 'no-store' });
    if (!r.ok) return null;
    const d = (await r.json())?.result?.USDTZUSD;
    const m = mid(d?.b?.[0], d?.a?.[0]);
    return m && m > 0.9 && m < 1.1 ? m : null;
  } catch { return null; }
}
...
export async function GET(request: Request) {
  try {
    ...
    // Every source in parallel, plus the USDT/USD rate. One slow venue must not
    // hold up the rest, so each gets its own short timeout and a failure just
    // drops that source.
    const [settled, rate] = await Promise.all([
      Promise.all(ALL.map(async (src) => {
        try {
      ...
      Promise.race([usdtUsd(), new Promise<null>((resolve) => setTimeout(() => resolve(null), 2500))]),
    ]);
    ...
    return NextResponse.json(
      { symbol: sym, sources, missing, usdtUsd: rate, time: Date.now() },
      { headers: { 'Cache-Control': 'no-store' } },
    );
  } catch (e) {
    console.error('[api/market/index-sources]', e);
    return NextResponse.json({ sources: {}, missing: ALL, usdtUsd: null, time: Date.now() }, { status: 200 });
  }
}
