        ...
        <div className="lg:col-span-5 flex flex-col">
          ...
          <div className="flex flex-col w-full mb-6">
            ...
            <div className="flex items-center justify-between w-full h-[104px] px-5 bg-slate-50 dark:bg-[#0B0F14] border border-slate-200 dark:border-white/5 dark:shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)] rounded-[16px] transition-all">
              ...
              <div className="flex flex-col items-end justify-center h-full shrink-0 relative" ref={dropdownRef}>
                
                {/* Token Selector Trigger */}
                <div 
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className="flex items-center gap-2 px-3 py-2 rounded-full border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 shadow-sm dark:shadow-none cursor-pointer mb-[14px] hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
                >
                ...
                {/* Token Dropdown Menu */}
                <div 
                  className={`absolute top-[48px] right-0 w-[220px] bg-white dark:bg-[#121821] border border-slate-200 dark:border-white/10 rounded-[16px] shadow-xl z-50 transition-all duration-200 ease-in-out origin-top-right ${isDropdownOpen ? 'scale-100 opacity-100 pointer-events-auto' : 'scale-95 opacity-0 pointer-events-none'}`}
                >
                  <div className="flex flex-col max-h-[280px] overflow-y-auto p-1.5 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-200 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full">
                    {TOKENS.map(token => (
                      <div 
                        ...
                        className={`flex items-center justify-between p-2.5 rounded-xl cursor-pointer transition-all duration-200 group ${activeToken === token ? 'bg-black/5 dark:bg-white/10' : 'hover:bg-[#00A8E8]/10'}`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[14px] font-bold text-slate-900 dark:text-white">{token}</span>
                        </div>
                        
                        {/* Token Balance logic fetch hook goes here */}
                        <div className="flex flex-col items-end">
                          <span className="text-[13px] font-semibold text-slate-600 dark:text-white/70">
