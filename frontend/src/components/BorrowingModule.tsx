  ...
  return (
    <div className="industrial-panel bg-surface">
      <div className="flex justify-between items-start mb-8">
        <div>
          <h3 className="text-[11px] font-bold uppercase tracking-wider text-black/30 dark:text-white/40">Credit Facility</h3>
      ...
      <div className="space-y-6">
        <div>
          <label className="text-[10px] font-bold text-black/40 dark:text-white/60 uppercase block mb-2">Deposit Collateral</label>
          <div className="relative">
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex gap-2">
              <button 
                ...
                className={`text-[10px] font-bold px-2 py-1 rounded transition-colors ${collateralType === 'USDT' ? 'bg-[#00A8E8] text-white' : 'bg-black/5 dark:bg-black/10 text-black/40 dark:text-white/40'}`}
              >
                USDT
              </button>
              <button 
                onClick={() => setCollateralType('USDC')}
                className={`text-[10px] font-bold px-2 py-1 rounded transition-colors ${collateralType === 'USDC' ? 'bg-[#00A8E8] text-white' : 'bg-black/5 dark:bg-black/10 text-black/40 dark:text-white/40'}`}
              >
                USDC
              </button>
            </div>
        ...
        <div className="bg-black/[0.02] dark:bg-white/[0.02] rounded-xl p-4 border border-[var(--border)]">
          <div className="flex justify-between mb-2">
            <span className="text-[10px] font-bold text-black/40 dark:text-white/60 uppercase">Max Borrowing Capacity</span>
