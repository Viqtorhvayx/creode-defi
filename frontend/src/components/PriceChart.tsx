...
export const PriceChart: React.FC<PriceChartProps> = ({ theme = 'light' }) => {
  ...
  // streams off Binance's trade prints via lib/binanceStream.ts — a tick per
  ...
  // source. Binance itself is reached through Creode's own server relay
  // (/api/market/binance-stream) rather than a direct client WebSocket,
  // since some browsers block raw WebSocket upgrades outright. Falls back
  // to Pyth, then Bybit, if the stream goes quiet.
